...
export default async function ImoveisPage({ searchParams }: { searchParams: Promise<{ mes?: string; ano?: string }> }) {
  ...
  const resumos = await Promise.all((empresas ?? []).map(async empresa => {
    ...
    // PAGAMENTOS = soma de tudo que foi pago no mês selecionado.
  ...
  // TOTAL = soma dos pagamentos de TODAS as empresas no mês selecionado
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-green-600 text-white rounded-xl p-5 mb-4 flex items-center justify-between gap-3 text-xl font-bold tracking-wide">
        ...
        <span className="flex items-center gap-2 sm:gap-4">
          <Link href={`/imoveis?mes=${mesAnterior}&ano=${anoAnterior}`} aria-label="Mês anterior"
          ...
          <Link href={`/imoveis?mes=${mesProximo}&ano=${anoProximo}`} aria-label="Próximo mês"
            className="w-8 h-8 flex items-center justify-center rounded-full bg-green-700 hover:bg-green-800 transition-colors text-2xl leading-none">
            ›
          </Link>
        </span>
